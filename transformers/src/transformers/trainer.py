...
class Trainer:
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
        # return ImbalancedDatasetSampler(self.train_dataset, generator=torch.manual_seed(42))
        # Build the sampler.
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        self.control.should_evaluate = self.control.should_save
        self.control.should_log = self.control.should_save
